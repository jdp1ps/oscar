    ...
    <div class="ui-user">
      ...
      <section class="users">
        ...
        <article v-for="(user,i) in usersFiltered" class="card xs" @click="handlerClick(user)"
                 :class="{'selected': selectedUser == user, 'error': user.error, 'warning': user.warning}">
          <h3 class="card-title">
      ...
      <section v-if="logs" class="logs">
        ...
        Identifiant : <code>{{ selectedUser.username }}</code><br>
        Email : <code>{{ selectedUser.email }}</code><br>
        Dernière connexion :
        <span v-if="selectedUser.lastLogin">
          <code>{{ $filters.dateFull(selectedUser.lastLogin) }}</code> à <code>{{ $filters.time(selectedUser.lastLogin) }}</code>
        </span>
        ...
        Personne associée : <code v-if="selectedUser.person">{{ selectedUser.person.displayname }}</code>
        <strong v-else class="text-danger">non trouvée</strong>
        <br>
        <div class="alert alert-danger" v-if="selectedUser.error">
          {{ selectedUser.error }}
        </div>
        <div class="alert alert-warning" v-if="selectedUser.warning">
          ...
          <section v-for="stack in logsFiltered">
            ...
            <article class="card" v-for="log in stack.logs">
              <div class="text-highlight">
                <small>{{ $filters.log(log.dateCreated.date) }}</small>
                - depuis l'IP <strong>{{ log.ip }}</strong>
              </div>
              <p v-html="$filters.log(log.message)"></p>
            </article>
          </section>
        </template>
        <section v-else>
...
export default {
  ...
  methods: {
    ...
    performDeleteRole() {
      ...
      AxiosOscar.deleteParams(
          this.urlRoles,
          { 'authentification_id': authentification_id, 'role_id': role_id })
          .then(
              res => {
    ...
    performAddRole() {
      ...
      if (this.addRoleUser.id && this.addRoleId) {
        ...
        AxiosOscar.post(this.urlRoles, form).then(
            res => {
    ...
    handlerClick(user) {
      ...
      this.loading = true;
      AxiosOscar.get(this.urlLogs + user.id).then(
          (res) => {
            this.logs = res.data;
          },
          err => {
    ...
    fetch(){
      AxiosOscar.get(this.url).then(ok=>{
        this.roles = ok.data.roles;
        this.users = ok.data.users;
        this.urlLogs = ok.data.urlLogs;
        this.urlRoles = ok.data.urlRoles;
      })
    }
  },
