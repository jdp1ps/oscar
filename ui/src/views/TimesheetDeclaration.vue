      ...
      <div class="overlay" v-if="commentEdited">
        <div class="overlay-content">
          ...
          <textarea name="comment" class="form-control" id="" cols="30" rows="10" v-model="commentEditedContent"
                    :class="{'disabled': !ts.editable }"></textarea>
          <nav class="buttons">
            <button class="btn btn-primary" @click="handlerSendComment" :class="{'disabled': !ts.editable }">
              <i class="icon-floppy"></i> Enregistrer le commentaire
            </button>
            <button class="btn btn-default" @click="commentEdited = null"><i class="icon-cancel-outline"></i>Annuler
            </button>
          </nav>
        </div>
      </div>
      ...
      <div class="content container overlay-content">
        ...
        <div v-else>
          <p>
            ...
            Journée : <strong>{{ $filters.date(selectedDay.date) }}</strong><br/>
          </p>
          ...
          <div class="row">
            ...
            <div class="col-md-6">
              ...
              <timechooser @timeupdate="handlerDayUpdated"
                           ...
                           :fill="fillDayValue"
                           :default-duration="dayMenuTime"></timechooser>
            </div>
            <div class="col-md-6">
              <h4>Commentaire</h4>
        ...
        <div v-else-if="selectedWeek">
          <h3 @click.shift="debug = selectedWeek" class="title-with-menu">
            ...
            <nav class="right-menu" v-if="ts.editable">
              <a href="#" @click="handlerCopyWeek(selectedWeek)" title="Copier les créneaux de la semaine"><i
                  class="icon-docs"></i></a>
              <a href="#" @click="handlerPasteWeek(selectedWeek)" title="Coller les créneaux" v-show="clipboardData"><i
                  class="icon-paste"></i></a>
            </nav>
          </h3>
            ...
            <section v-for="periodValidation in ts.periodsValidations" class="card card-xs">
              ...
              <a href="#" @click="rejectPeriod = periodValidation"
                 v-if="periodValidation.status == 'conflict'
                               && (periodValidation.rejectadmin_message || periodValidation.rejectsci_message || periodValidation.rejectactivity_message )">Détails
                sur le rejet</a>
            </section>
          </div>
...
export default {
  ...
  props: {
    // Options
    declarationInHours: {type: Boolean, default: false},
    defaultYear: {type: Number, default: (new Date()).getFullYear()},
    defaultMonth: {type: Number, default: (new Date()).getMonth()},

    // Required
    url: {type: String, required: true},
    urlValidation: {type: String, required: true},
    urlImport: {type: String, required: true},
  },
  ...
  methods: {
    ...
    /**
     * Soumission de créneaux.
     * @param datas
     */
    performAddDays(datas) {
      let send = {};
      send.timesheets = JSON.parse(JSON.stringify(datas));
      send.action = 'add';
