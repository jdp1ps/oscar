  ...
  <div class="day-details" :class="{'locked': day.locked}">
    ...
    <h3 @click.stop.prevent.shift="$emit('debug', day)" class="title-with-menu">
      ...
      <nav class="right-menu">
        <a href="#" @click.prevent="$emit('copy', day)" v-show="day.othersWP || day.declarations.length"
           title="Copier les créneaux">
          <i class="icon-docs"></i>
        </a>
    ...
    <div class="alert alert-danger" v-show="day.total > day.maxLength">
      <i class="icon-attention"></i> Le temps déclaré <strong>excède la durée autorisée</strong>. Vous ne pourrez pas
      soumettre votre feuille de temps.
    </div>

    <div v-if="editable">
      Compléter avec :
      <wpselector :others="others" :workpackages="workPackages" @select="addToWorkpackage"
                  :selection="selection"></wpselector>
    </div>
    ...
    <section>
      ...
      <template v-if="day.declarations.length">
        ...
        <article class="wp-duration card xs">
          ...
          <div class="total">
            <span class="text-large text-xl">{{ $filters.duration2(totalWP, day.dayLength) }}</span>
          </div>
          <div class="left"></div>
        </article>
        <hr>
      ...
      <section class="othersWP" v-if="day.othersWP">
        ...
        <article class="wp-duration card xs" v-for="t in day.othersWP">
          ...
          <div class="total">{{ $filters.duration2(t.duration, day.dayLength) }}</div>
          <div class="left buttons-icon">
            <i class="icon-trash" @click="$emit('removetimesheet', t)"
               :class="day.editable != true ? 'disabled':''"></i>
            <i class="icon-edit" @click="$emit('edittimesheet', t, day)"
               :class="day.editable != true ? 'disabled':''"></i>
          </div>
        </article>
        <hr>
      ...
      <article class="wp-duration card xl">
        ...
        <div class="total">{{ $filters.duration2(totalJour, day.dayLength) }}</div>
      </article>
    </section>
  </div>
</template>
