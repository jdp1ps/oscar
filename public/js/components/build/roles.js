...
var Roles = Vue.extend({
    ...
    computed: {
        roles() {
            ...
            this.common.roles.forEach(role => {
                if ((role.spot & this.activeSpots) > 0 || role.spot == 0) filteredRoles.push(role);
            });
    ...
    methods: {
        ...
        remove(role) {
            Bootbox.confirm({
                ...
                callback: response => {
                    if (!response) return;
                    this.loading = true;
                    this.$http.delete(this.$http.$options.root + '/' + role.id).then(res => {
        ...
        save: function (e) {
            this.loading = true;
            if (this.form.id) {
                this.$http.put(this.$http.$options.root + '/' + this.form.id, this.form).then(res => {
                    for (let i = 0; i < this.common.roles.length; i++) {
                ...
                this.$http.post(this.$http.$options.root, this.form).then(res => {
        ...
        updateRoleSpot: function (role, val) {
            ...
            this.$http.put(this.$http.$options.root + '/' + role.id, data).then(res => {
