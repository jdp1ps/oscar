...
var Timesheet = Vue.extend({
    ...
    data(){
        return {
            ...
            tagslabels: [],
            mode: [
                {key: "day", label: "Journalier", format: "dddd Do MMMM YYYY"},
                {key: "week", label: "Hebdomadaire", format: "Wo en YYYY"},
                {key: "month", label: "Mensuel", format: "MMMM YYYYY"},
                {key: "year", label: "Annuel", format: "YYYY"}
            ]
        }
    },
    ...
    methods: {
        ...
        remove(item){
            ...
            if (this.events.indexOf(item) > -1)
                this.events.splice(this.events.indexOf(item), 1);
        },

        send(){
            this.http.post(this.url, JSON.parse(JSON.stringify(this.events))).then(
                (res) => {
                    console.log(res)
                },
                (err) => {
