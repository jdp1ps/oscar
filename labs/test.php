...
$test->addStyle("labelTitle", $labelTitle);
...
$test->addStyle("labelValue", $labelValue);
...
$test->addStyle("headResearch", $headResearch);
...
$test->addStyle("headEducation", $headEducation);
...
$test->addStyle("headOther", $headOther);
...
$test->addStyle("withValue", $withValue);
...
$test->addStyle("cellTotalBottom", $cellTotalBottom);
...
$test->addStyle("personComment", $personComment);
...
$test->addStyle("totalColumn", $totalColumn);
...
$test->getActiveSheet()->getRowDimension($test->getCurrentLine())->setRowHeight(40);
$test->drawCell("FEUILLE de TEMPS", $fullWidth, true, 'entete');
$test->nextLine();
$test->getActiveSheet()->getRowDimension($test->getCurrentLine())->setRowHeight(30);
$test->drawCell($datas['activity']['label'], $fullWidth, true, 'entete');
$test->nextLine();
$test->nextLine();


$test->getActiveSheet()->getRowDimension($test->getCurrentLine())->setRowHeight(4);
$test->drawCell("", $fullWidth, true, 'labelTitle');
$test->nextLine();

$test->getActiveSheet()->getRowDimension($test->getCurrentLine())->setRowHeight(20);
$test->drawCell("", 0, true, 'labelTitle');
$test->drawCell("Début : ", $sizing, true, 'labelTitle');
$test->drawCell($datas['period']['startLabel'], $sizing, true, 'labelValue');
$test->drawCell("", 0, true, 'labelTitle');
$test->drawCell("Fin : ", $sizing, true, 'labelTitle');
$test->drawCell($datas['period']['endLabel'], $sizing, true, 'labelValue');
$test->drawCell("", 0, true, 'labelTitle');
$test->nextLine();
...
$test->getActiveSheet()->getRowDimension($test->getCurrentLine())->setRowHeight(20);
$test->drawCell("", 0, true, 'labelTitle');
$test->drawCell("PFI : ", $sizing, true, 'labelTitle');
$test->drawCell($datas['activity']['PFI'], $sizing, true, 'labelValue');
$test->drawCell("", 0, true, 'labelTitle');
$test->drawCell(" : ", $sizing, true, 'labelTitle');
$test->drawCell("", $sizing, true, 'labelValue');
$test->drawCell("", 0, true, 'labelTitle');
$test->nextLine();
...
$test->getActiveSheet()->getRowDimension($test->getCurrentLine())->setRowHeight(20);
$test->drawCell("", 0, true, 'labelTitle');
$test->drawCell("Acronyme : ", $sizing, true, 'labelTitle');
$test->drawCell($datas['activity']['projectacronym'], $sizing, true, 'labelValue');
$test->drawCell("", 0, true, 'labelTitle');
$test->drawCell("N°OSCAR : ", $sizing, true, 'labelTitle');
$test->drawCell($datas['activity']['numOscar'], $sizing, true, 'labelValue');
$test->drawCell("", 0, true, 'labelTitle');
$test->nextLine();
...
$test->getActiveSheet()->getRowDimension($test->getCurrentLine())->setRowHeight(30);
$test->drawCell(" ", 0, true);

// LOTS
foreach ($datas['wps'] as $wp) {
    $test->drawCell($wp['code'], 0, true, 'headResearch');
}
$test->drawCell('Total', 0, true, 'headResearch');

foreach ($datas['ces'] as $ce) {
    $test->drawCell($ce, 0, true, 'headResearch');
}
foreach ($datas['othersGroups']['research'] as $r) {
    $test->drawCell($r['label'], 0, true, 'headResearch');
}

$test->drawCell('Total', 0, true, 'headResearch');

foreach ($datas['othersGroups']['education'] as $r) {
    $test->drawCell($r['label'], 0, true, 'headEducation');
}

foreach ($datas['othersGroups']['abs'] as $r) {
    $test->drawCell($r['label'], 0, true, 'headAbs');
}

foreach ($datas['othersGroups']['other'] as $r) {
    $test->drawCell($r['label'], 0, true, 'headOther');
}

$test->drawCell('TOTAL', 0, true, 'person');

$colSign = $test->getCurrentCol();
$test->drawCell('Signature', 0, true, 'person');
...
foreach ($datas['foo'] as $person=>$line) {
    ...
    // Recherche
    $test->drawCell($person, 0, true, 'person');
    foreach ($datas['wps'] as $wp) {
        $code = $wp['code'];
        $test->getCurrentStyle()->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB("ff$colorResearchBG");
        $test->drawCell($line['main'][$code], 0, true, $line['main'][$code] ? 'withValue' : 'noValue');
    }

    $test->drawCell($line['totalMain'], 0, true, 'totalColumn');

    foreach ($datas['ces'] as $ce) {
        $test->getCurrentStyle()->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB("ff$colorResearchBG");
        $test->drawCell($line['ce'][$ce], 0, true, $line['ce'][$ce] ? 'withValue' : 'noValue');
    }

    foreach ($datas['othersGroups']['research'] as $r) {
        $test->getCurrentStyle()->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB("ff$colorResearchBG");
        $test->drawCell($line['others'][$r['code']], 0, true, $line['others'][$r['code']] ? 'withValue' : 'noValue');
    }
    $test->getCurrentStyle()->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB("ff$colorResearchBG");
    $test->drawCell($line['totalResearch'], 0, true, 'totalColumn');

    foreach ($datas['othersGroups']['education'] as $r) {
        $test->getCurrentStyle()->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB("ff$colorEducationBG");
        $test->drawCell($line['others'][$r['code']], 0, true, $line['others'][$r['code']] ? 'withValue' : 'noValue');
    }

    foreach ($datas['othersGroups']['abs'] as $r) {
        $test->getCurrentStyle()->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB("ff$colorAbsBG");
        $test->drawCell($line['others'][$r['code']], 0, true, $line['others'][$r['code']] ? 'withValue' : 'noValue');
    }

    foreach ($datas['othersGroups']['other'] as $r) {
        $test->getCurrentStyle()->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB("ff$colorOtherBG");
        $test->drawCell($line['others'][$r['code']], 0, true, $line['others'][$r['code']] ? 'withValue' : 'noValue');
    }

    $test->drawCell($line['totaux']['total'], 0, true, 'totalColumn');
    $test->drawCell(' ', 0, true, 'person');
    $test->nextLine();
}
...
foreach ($datas['wps'] as $wp) {
    ...
    $test->drawCell($value, 0, true, 'cellTotalBottom');
}
$test->drawCell($totaux['totalMain'], 0, true, 'cellTotalBottom');
...
$test->drawCell($totaux['totalResearch'], 0, true, 'cellTotalBottom');
...
$test->drawCell($totaux['total'], 0, true, 'cellTotalBottom');
...
foreach ($datas['foo'] as $person=>$line) {
    $test->getActiveSheet()->getRowDimension($test->getCurrentLine())->setRowHeight(60);
    $test->drawCell("", 0, true );
    $test->drawCell($person, $widthPerson, true, 'personComment');
    $test->drawCell("...", $widthComment, true, 'comment');
...
$test->getActiveSheet()->getColumnDimension($colSign)->setAutoSize(false)->setWidth('20');

$test->getActiveSheet()->getPageSetup()
    ->setFitToPage(true)
    ->setOrientation(\PhpOffice\PhpSpreadsheet\Worksheet\PageSetup::ORIENTATION_LANDSCAPE)
    ->setPaperSize(\PhpOffice\PhpSpreadsheet\Worksheet\PageSetup::PAPERSIZE_A4);

$test->generate('TEST.xlsx');
die();