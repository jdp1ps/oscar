...
class OscarAuthListCommand extends OscarCommandAbstract
{
    ...
    protected function configure()
    {
        $this
            ...
            ->addOption(
                ...
                'Champ utilisé pour le trie (' . implode(', ', array_values($this->getFields())) . ')',
                'id'
            )
            ->addOption(
                'search',
                'e',
                InputOption::VALUE_OPTIONAL,
                'Expression de recherche sur le nom/email/identifiant'
            )
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if (!(in_array($fieldSortInput, array_keys($this->getFields())) || in_array(
                $fieldSortInput,
                array_values($this->getFields())
            ))) {
            $output->writeln(
                "<error>Le champ $fieldSortInput n'est pas valide pour le trie, les champs valides sont : " . implode(
                    ", ",
                    array_values($this->getFields())
                ) . "</error>"
            );
        ...
        foreach ($oscaruserContext->getAuthentifications($options) as $authentification) {
            ...
            $rows[] = [
                ...
                $dateLogin,
                $authentification['IDPERSON'] ? sprintf(
                    "<id>%s</id>",
                    $authentification['IDPERSON']
                ) : "<none>Non</none>"
            ];
        }
        $table->setRows($rows);
