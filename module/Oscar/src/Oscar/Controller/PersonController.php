...
class PersonController extends AbstractOscarController
{
    ...
    public function syncHarpegeAction()
    {
        $idPerson = $this->params()->fromRoute('id', null);
        ...
        if($idPerson){
            // Liste des personnes dans Oscar (Par IDHARPEGE)
            die("TODO : Synchronisation Harpège pour une seule personne");
        } else {
            $persons = [];
            $async = [];
            $datasUpdated = [];
            ...
            while ($row = oci_fetch_array($personsHarpege, OCI_ASSOC + OCI_RETURN_NULLS)) {

                $harpegeId = $row['HARPEGEID'];

                if( !isset($persons[$harpegeId]) ){
                    if( !isset($datasCreated[$harpegeId]) ){
                    ...
                    if( !isset($datasUpdated[$harpegeId]) ){
            ...
            foreach( $datasUpdated as $id=>$d ){
                /** @var Person $person */
                $person = $d['person'];

                if( $d['INM'] !== $person->getHarpegeINM() || $person->getEmailPrive() != $d['EMAIL'] ){
                    $person->setHarpegeINM($d['INM'])
                        ->setEmailPrive($d['EMAIL']);
                    $this->getEntityManager()->flush($person);
                    $outUpdated[] = "La personne ".$person->log()." a été mise à jour.";
                } else {
                    unset($d['INM']);
                }
            }

            // On parse les données à créer voir si une mise à jour est necessaire
            foreach( $datasCreated as $id=>$p ){
                $person = new Person();
                $this->getEntityManager()->persist($person);
                $person->setFirstname(ucfirst($p['PRENOM']))
                    ->setLastname(ucfirst($p['NOM']))
                    ->setCodeHarpege(ucfirst($id))
                    ->setCodeLdap($harpege->getLdapIdFromHarpegeId($id))
                    ->setEmailPrive($p['EMAIL'])
                    ->setHarpegeINM($p['INM']);
                $outCreated[] = "La personne ".$person->log()." a été ajoutée dans oscar.";
                $this->getEntityManager()->flush($person);
            }
